...
void CHuffman::InitTable(BYTE *buf, long size)
{
	...
	if (ListIsEmpty())
	{
		MessageBoxW(NULL, L"Error InitTable", L"Error", MB_OK);
		ExitProcess(-1);
	}
	...
	if (!table[ESC].count)
	{
		MessageBoxW(NULL, L"ESC symbol is too long and concatinated to zero", L"Error", MB_OK);
		ExitProcess(-1);
	}
}
...
int CHuffman::FindKey(BYTE *buf)
{
	...
	MessageBoxW(NULL, L"Invalid key was not found", L"Error", MB_OK);
	ExitProcess(-1);
	return -1;
}
	...
	for (i = 0; i < tlen; i++)
	{
		...
		if (status)
		{
			...
			else
			{
				...
				{
					/*
					FatalError("Error in encoder. Can`t be zero size sequence\r\n"
					"i = %d, sp = %d, so = %d", i, sp, so);
					*/
					MessageBoxW(NULL, L"Encoder Error", L"Error", MB_OK);
					ExitProcess(-1);
				}
			}
		}
